...
def signal_labeller(data, takeprofit):
    ...
    for i in range(len(data.index)):
        cur_price = data['open'].iloc[i]
        long_bound_hit = data.iloc[i+1:].where(data.high >= cur_price + takeprofit*pip).first_valid_index()
        short_bound_hit = data.iloc[i+1:].where(data.low <= cur_price - takeprofit*pip).first_valid_index()
...
##Patrick
data_filename = "EURUSD_M1_201701020000_202207292356.csv"
...
export_folder = "C:\\Users\\Patrick\\Documents\\UNI - USYD\\2022 - Capstone\\Python Backtesting System\\github versions\\Live\\Python-Backtesting-vPM\\Labelling project\\Individual Runs\\EURUSD_M1_V2"
